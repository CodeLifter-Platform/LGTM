...
- Azure DevOps interactions go through the provided SDK client; do not invoke the `az` CLI or shell out to `git push` to a remote you weren't given.
...
- Azure DevOps tooling is exposed via the configured Azure DevOps integration; use those operations rather than shelling out.
...
- Azure DevOps operations use the provided SDK; do not generate REST URLs by hand.
